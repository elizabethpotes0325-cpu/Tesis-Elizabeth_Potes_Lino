...
const int PinA_left= 2; // Pin para la senal A del encoder
const int PinA_right=3; // Pin para la senal B del encoder
...
//////////////////////PID parameter///////////////////////////////
double kp = 34.5, ki = 0.9, kd = 0.62;                   //angle loop parameter
double kp_speed = 3.65, ki_speed = 0.08, kd_speed = 0;   // speed loop parameter
double kp_turn = 24.4, ki_turn = 0, kd_turn = 0.09;                 // steering loop parameter
...
//Serial.println(count_right);
  ...
  //PinA_right Level change triggers the external interrupt; execute the subfunction Code_right
}
...
void countpluse()
{
  ...
  Serial.println(lpluse);

  if ((pwm1 < 0) && (pwm2 < 0))                     //judge the cars moving direction; if backward (PWM namely motor voltage is negative) pulse is a negative number.
  {
    rpluse = -rpluse;
    lpluse = -lpluse;
  }
  ...
  else if ((pwm1 < 0) && (pwm2 > 0))                 \//judge the cars moving direction; if turn left - right pulse is a positive number; left pulse is a negative number.
  {
    rpluse = rpluse;
    lpluse = -lpluse;
  }
  else if ((pwm1 > 0) && (pwm2 < 0))               //judge the cars moving direction; if turn right, right pulse is a negative number; left pulse is a positive number.
  {
    rpluse = -rpluse;
    lpluse = lpluse;
  }
...
void DSzhongduan()
{
  ...
  cc++;
  if(cc>=8)     //5*8=4 enter PI algorithm of speed per 40ms
  {
    speedpiout();   
    cc=0;  //Clear
  }
...
void anglePWM()
{
  ...
  if(angle>45 || angle<-45)      //if tilt angle is greater than 45 grados, motor will stop
  {
    pwm1=pwm2=0;
  }
